...
#[tokio::test]
async fn remote_control_transport_refreshes_server_token_before_expiry() {
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("listener should bind");
    let remote_control_url = remote_control_url_for_listener(&listener);
    ...
    let expires_at = (OffsetDateTime::now_utc() + time::Duration::seconds(31))
        .format(&Rfc3339)
        .expect("expiration timestamp should format");
    respond_with_json(
        enroll_request.stream,
        remote_control_server_token_response_with_expires_at(
            "srv_e_test",
    ...
    timeout(Duration::from_secs(5), first_websocket.next())
        .await
        .expect("first websocket should close when server token refresh is due");
    drop(first_websocket);
